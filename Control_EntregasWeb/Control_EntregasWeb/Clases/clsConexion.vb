...
Public Class clsConexion
    Dim vnConn As New SqlConnection
    Dim vcCmm As New SqlCommand
    Dim vrRdr As SqlDataReader
    ...
    Public Function fAbreConexion() As Boolean
        ...
        Try
            ...
            vnConn.ConnectionString = ConfigurationManager.AppSettings("cnfConexion")
    ...
    Public Function fEjecutaQueryDS(ByVal psQuery As String) As Data.DataSet
        ...
        If fAbreConexion() Then
            ...
            vaAdp = New SqlDataAdapter(vcCmm)
    ...
    Public Function fEjecutaQueryRD(ByVal psQuery As String) As SqlDataReader
    ...
    Public Function fEjecutaQueryRDParam(ByVal psQuery As String, parameters As String) As SqlDataReader
        'Ejecuta query y retorna el reader
        If fAbreConexion() Then
            vcCmm.Connection = vnConn
            vcCmm.CommandType = Data.CommandType.Text
            ...
            If parametersArray(0).Length > 0 AndAlso parametersArray.Length = 1 Then
                vcCmm.Parameters.Add(New SqlParameter("@p" + (abcParam).ToString(), SqlDbType.VarChar)).Value = parametersArray(0)
            ElseIf parametersArray(0).Length > 0 AndAlso parametersArray.Length > 1 Then
                For Each param In parametersArray
                    vcCmm.Parameters.Add(New SqlParameter("@p" + (abcParam).ToString(), SqlDbType.VarChar)).Value = parametersArray(numberParam)
                    numberParam += 1
    ...
    Private Function getCharToNumber(ByVal index As Integer) As String
        Const abcString As String = "abcdefghijklmnopqrstuvwxyz" 'letras del abecedario
    ...
    Public Function fdEjecutaSPDatos(ByVal psSP As String, ByVal psParametros() As String) As DataSet
        ...
        Dim ocmdComando As New System.Data.SqlClient.SqlCommand
        Dim vadAdapter As New System.Data.SqlClient.SqlDataAdapter
        ...
        If fAbreConexion() Then
            With ocmdComando
                ...
                For piPar As Integer = 0 To psParametros.Length - 1 'en sql no utiliza refcursors
                    ...
                    If vsValorPar(1) = 1 Then
                        .Parameters.Add(vsValorPar(0), SqlDbType.Decimal)
                    ElseIf vsValorPar(1) = 2 Then
                        '.Parameters.Add(vsValorPar(0), OracleClient.OracleType.VarChar, 4000)
                        .Parameters.Add(vsValorPar(0), SqlDbType.VarChar, 10000)
                    ElseIf vsValorPar(1) = 3 Then
                        .Parameters.Add(vsValorPar(0), SqlDbType.Date)
                    End If
            ...
            vadAdapter = New SqlDataAdapter(ocmdComando)
